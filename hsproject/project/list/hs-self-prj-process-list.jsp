...
<link rel="stylesheet" type="text/css" href="/js/jquery-autocomplete/browser_wev8.css" />
	...
	<BODY>
		...
		<FORM id=report name=report action="/hsproject/project/list/hs-prj-process-list.jsp" method=post>
			...
			<div class="advancedSearchDiv" id="advancedSearchDiv" style="display:none;">
				...
				<wea:group context="查询条件">
					...
					<wea:item>
						<input name="processname" id="processname" class="InputStyle" type="text" value="<%=processname%>"/>
					</wea:item>
					<wea:item>阶段类型</wea:item>
					<wea:item>
						<brow:browser viewType="0"  name="processtype" browserValue="<%=processtype%>"
						...
						width="120px"
						linkUrl=""
						browserSpanValue="<%= new BrowserInfoUtil().getProcessType(processtype)%>">
						</brow:browser>
               	   </wea:item>
					<wea:item>所属项目</wea:item>
					<wea:item>
						...
						width="120px"
						linkUrl=""
						browserSpanValue="<%= new ProjectInfoDao().getPrjName(prjid)%>">
						</brow:browser>
               	   </wea:item>
		...
		String sqlWhere =  " where 1=1 and fzrid='"+userid+"'";

		String orderby =  " prjid desc "  ;
		if(!"".equals(processname)){
			sqlWhere = sqlWhere+" and upper(processname) like upper('%"+processname+"%')";
		}
		if(!"".equals(processtype)){
			sqlWhere = sqlWhere+" and processtype='"+processtype+"'";
		}
		if(!"".equals(prjid)){
			sqlWhere = sqlWhere+" and prjid='"+prjid+"'";
		}
		if(!"".equals(prjtype)){
		...
		"			<head>";
				tableString +="<col width=\"12%\" text=\"阶段名称\" column=\"processname\" orderkey=\"processname\" linkvaluecolumn=\"id\" linkkey=\"id\" href= \"/hsproject/project/view/hs-process-info-url.jsp\" target=\"_fullwindow\"/>"+ 
						...
						"<col width=\"12%\" text=\"所属项目\" column=\"prjid\" orderkey=\"prjid\" transmethod='hsproject.dao.ProjectInfoDao.getPrjNameWithLink' />"+
