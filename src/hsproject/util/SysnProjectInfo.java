...
public class SysnProjectInfo {
	...
	//	String xmjlemail = "";// 项目经理邮箱
		...
		int xmstatus = -1;
		String sql_dt = "";
		String sql = "select name,xmcode,xmjlname,xmgs,xmbm,sqrq,xmzl,firsttype,xmlb,lxdh,xmjlemail,sfndys,lxlzxs,xmtzysje,xmrjje,xmfyzyyjje,xmqtjf,xmyzyje,xmysyfy,xmstatus from wasu_projectbase where xmcode in (select prjcode from uf_prj_type_mid)";
		rs.executeSql(sql);
		while(rs.next()) {
	...
	public void sysprocess() {
		...
		String status = "";
		String xmprjid = "";
		sql="select id,procode,begindate,status from hs_projectinfo where procode in (select prjcode from uf_prj_type_mid)";
		rs.executeSql(sql);
		while(rs.next()) {
	...
	public void sysndoc() {
		...
		String begindate = "";
		String xmprjid = "";
		sql="select id,procode,begindate from hs_projectinfo where procode in (select prjcode from uf_prj_type_mid)";
		rs.executeSql(sql);
		while(rs.next()) {
			prjid = Util.null2String(rs.getString("id"));
			procode = Util.null2String(rs.getString("procode"));
