...
public class SysnoUtil {
	...
	public String getMaxNum(String tableName){
		...
		int count_cc = 0;
		String sql = "select count(id) as count_cc from uf_prj_sysno where strsys=null and " 
		...
		int num = 1;
		if(count_cc > 0){
			sql = "select nextseq from uf_prj_sysno where strsys=null and " 
					 +" datesys=null and tableName='"+tableName+"'";  
			rs.executeSql(sql);
			if(rs.next()){
			...
			sql = "update uf_prj_sysno set nextseq=nextseq+1 where strsys=null and " 
					 +" datesys=null and tableName='"+tableName+"'";
			rs.executeSql(sql);
	...
	public String getNum(String str,String tableName,int index){
		...
		int count_cc = 0;
		String sql = "select count(id) as count_cc from uf_prj_sysno where strsys='"+str+"' and " 
		...
		if(count_cc > 0){
			sql = "select nextseq from uf_prj_sysno where strsys='"+str+"' and " 
					 +" datesys='"+now+"' and tableName='"+tableName+"'";  
			rs.executeSql(sql);
			if(rs.next()){
				num = rs.getInt("nextseq");
			...
			sql = "update uf_prj_sysno set nextseq=nextseq+1 where strsys='"+str+"' and " 
					 +" datesys='"+now+"' and tableName='"+tableName+"'";
			rs.executeSql(sql);
