...
public class SysnProjectInfo2 {
	...
	//	String xmjlemail = "";// 项目经理邮箱
		...
		while(rs.next()) {
			...
			if(!"".equals(lxlzxs)) {
				pidComMap.put("argumentform",String.valueOf(Util.getIntValue(lxlzxs,1)+42));//换
			}
	...
	public void updateProcess(String prjid,String xmprjid,String status) {
		...
		while(rs.next()) {
			...
			if("1".equals(isover)){
				sql_dt = "update hs_prj_process set isdone='1',enddate='"+stonedate+"',status='完成',iscomplete='21' where processname='"+stonename+"' and prjid="+prjid;//换
			}else {
		...
		if(!"".equals(status)) {			
			...
			while(rs.next()) {
				...
				sql_dt = "update hs_prj_process set isdone='1',status='完成',iscomplete='21' where id="+Util.null2String(rs.getString("id"));//换
				rs_dt.executeSql(sql_dt);
			}
