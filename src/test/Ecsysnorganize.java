...
public class Ecsysnorganize {
	...
	public void operDepartment(){
		...
		String sql="select departmentcode,departmentname,departmentmark,(select departmentcode from hrmdepartment where id=a.supdepid) as superCode,(select subCompanyCode from hrmsubcompany where id=a.subcompanyid1) as subcompanyCode,canceled,showorder from hrmdepartment a where a.departmentcode is not null order by a.id asc";
	...
	public void operResource(){
		...
		while(rsd.next()){
			...
			if(rsd_dt1.next()){
				...
				hrb.addCusMap("field0", field0);
			}
			ReturnInfo result=hoa.operResource(hrb);
			 if(result.isTure()){
					
				}else{
					log.writeLog("人员同步岗位失败:"+Util.null2String(rsd.getString("workcode"))+" result:"+result.getRemark());
				}
